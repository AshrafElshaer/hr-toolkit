...
create or replace function get_user_role() returns user_role_enum as $$
...
create or replace function is_user_member() returns boolean as $$
    select user_role = 'member' from users where id = auth.uid(); 
$$ language sql stable;

create or replace function is_user_organization_admin(organization_id uuid) returns boolean as $$
    select admin_id = auth.uid() from organizations where id = organization_id;
$$ language sql stable;

create or replace function is_user_organization_member(organization_id uuid) returns boolean as $$
