create table public.attendance (
  ...
  constraint attendance_department_id_fkey foreign key (department_id) references departments (id) on delete cascade
...
select
  to public using (
    select
      auth.uid () = user_id
      or current_user_department_id () = department_id
